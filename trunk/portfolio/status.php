...
<title>Web Designer Brian Beloat - Projects Page</title>
...
<div class="container">
  ...
  <div class="content">
    <?
    	...
    	{
    		$query = "SELECT * FROM Status WHERE clientID = $idUser";
    	}
	...
	if ($numResults == 0) {
    echo '<br /><div id="jobTitle">'."You have no pending or completed projects available at this time".'</div>'."\n";
    exit;
}
...
while ($row = mysql_fetch_assoc($result)) {
	...
	$convDate = strtotime($row["jobETA"]);
	echo '<div id="jobTitle">'."\n".'<b>'.$row["jobName"]." - Due Date: " . date('M d, Y',$convDate) . "</b></br></div>"."\n";
    echo '<div id="jobText"><b>Info: </b>'.$row["jobInfo"]. "<br />";
    echo '<b>Notes:</b><br />'.$row["jobNotes"]. "<br /><br />";
    echo '<b>Job Complete %: </b>'.$jobStatus. "<br />";
    if ($jobStatus == 100)
    {
    	echo 'URL: <a class="jobTitle" href="'.$row["jobURL"].'">'.$row["jobURL"]."</a><br />";	
    }
	if ($type == "admin")
	{
		echo "<br /><a class='jobTitle' href='updateJobForm.php?jobID=". $row["jobID"] ."'>";
		echo 'Edit/Delete this job?</a>'."\n".'<br />'."\n";
	}
    echo '</div><br />';    
}
