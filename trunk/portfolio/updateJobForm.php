...
<title>Web Designer Brian Beloat - Update Project</title>
...
<?
	...
	$sql = "SELECT * FROM Status WHERE jobID='$jobID'";
...
?>
<div id="jobText">
<b>Update/Delete Project Entry</b><br /><br />
<form id="jobForm" action="updateJob.php" method="post">
<input id="jobID" name="jobID" value="<? echo $row["jobID"]; ?>" type="hidden" />
<p>
<label for="Title">Job Name</label>
<input name="jobName" id="jobName" value="<? echo $row["jobName"]; ?>" class="required">
</p>
<p>
		<label for="Title">Client ID</label>
		<input name="clientID" id="clientID" value="<? echo $row["clientID"]; ?>" class="required">
		</p>
<p>
<label for="jobInfo">Info</label>
<textarea cols=100 rows=3 name="jobInfo" id="jobInfo">
<? echo $row["jobInfo"]; ?>
</textarea>
</p>
<p>
<label for="jobNotes">Notes</label>
<textarea cols=100 rows=10 name="jobNotes" id="jobNotes">
<? echo $row["jobNotes"]; ?>
</textarea>
</p>
<p>
<label for="jobETA">ETA (Format: YYYY-MM-DD)</label>
<input name="jobETA" id="jobETA" value="<? echo $row["jobETA"]; ?>">
</p>
<p>
<label for="jobStatus">Percentage Complete (Whole Number 0-100)</label>
<input name="jobStatus" id="jobStatus" maxlength="3" value="<? echo $row["jobStatus"]; ?>">
</p>
<p>
<label for="jobURL">URL (full address)</label>
<input name="jobURL" id="joburl" value="<? echo $row["jobURL"]; ?>">
</p>
<p>
